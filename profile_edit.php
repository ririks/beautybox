...
if (isset($_POST['simpan'])) {
    ...
    $select_user = mysqli_query($conn, "SELECT * FROM users_detail WHERE user_id = '$user_id'") or die('Query failed');
    if (mysqli_num_rows($select_user) > 0) {
        // Update the existing address
        $query = "UPDATE users_detail SET full_name = '$name', number = '$number', address = '$address', subdistrict = '$subdistrict', city = '$city', province = '$province', country = '$country', post_code = '$post_code' WHERE user_id = '$user_id'";
    } else {
